...
from available_datasets.multimodal.vqa.coco_qa import MAX_NR_OF_UNIQUE_ANSWERS_FOR_COCO_QA
...
class FusionType(Enum):
    ...
    EARLY_WITH_IMAGE_CLS_AND_POOLING_MEAN = 'early_with_image_cls_and_pooling_mean'
    LATE_WITH_IMAGE_CLS_AND_POOLING_MEAN = 'late_with_image_cls_and_pooling_mean'


centralized_base_model = None


def _initialize_base_model(use_pre_layer_norm, use_post_layer_norm, use_large_encoder, freeze_encoder, nr_of_encoder_blocks_to_finetune, trainable_params_key, use_clip_encoder_for_text_embeddings, device, fusion_type, num_classes):
...
def get_split_model(use_pre_layer_norm, use_post_layer_norm, use_large_encoder, freeze_encoder, nr_of_encoder_blocks_to_finetune, trainable_params_key, use_clip_encoder_for_text_embeddings, device, fusion_type, num_classes):
    _initialize_base_model(
        use_pre_layer_norm=use_pre_layer_norm,
        use_post_layer_norm=use_post_layer_norm,
        use_large_encoder=use_large_encoder,
...
class CentralizedModel(MetaTransformerBase):
    ...
    def __init__(self, use_pre_layer_norm, use_post_layer_norm, use_large_encoder, freeze_encoder, nr_of_encoder_blocks_to_finetune, trainable_params_key, use_clip_encoder_for_text_embeddings, device, fusion_type, num_classes=MAX_NR_OF_UNIQUE_ANSWERS_FOR_COCO_QA):
        super().__init__(num_classes=num_classes, trainable_params=get_trainable_params_keys('classifier_and_image_tokenizer', trainable_params_key), use_pre_layer_norm=use_pre_layer_norm, use_post_layer_norm=use_post_layer_norm, use_large_encoder=use_large_encoder, freeze_encoder=freeze_encoder, nr_of_encoder_blocks_to_finetune=nr_of_encoder_blocks_to_finetune)
        ...
        self.fusion_type = FusionType.EARLY_WITH_IMAGE_CLS_AND_POOLING_MEAN if fusion_type == 'default' else FusionType[fusion_type.upper()]

        self.setup_tokenizers_for_fusion(use_clip_encoder_for_text_embeddings, use_large_encoder, device)
    ...
    def pre_load_tokenizers_and_norm_layer_weights(self):
        ...
        if self.use_pre_layer_norm and 'ln_pre.weight' in tokenizer_weights:
        ...
        if self.use_post_layer_norm and 'ln_post.weight' in tokenizer_weights:
    ...
    def setup_tokenizers_for_fusion(self, use_clip_encoder_for_text_embeddings, use_large_encoder, device):
        match self.fusion_type:
            case FusionType.EARLY_WITH_IMAGE_CLS_AND_POOLING_MEAN | FusionType.LATE_WITH_IMAGE_CLS_AND_POOLING_MEAN:
                ...
                self.text_tokenizer = Data2Seq(modality=InputModality.TEXT, embed_dim=self.embed_dim, use_clip_encoder_for_text_embeddings=use_clip_encoder_for_text_embeddings, use_large_encoder=use_large_encoder, device=device)
    ...
    def forward(self, x):
        ...
        x = self.forward_with_fusion(text, image)
    ...
    def forward_with_fusion(self, text, image):
        batch_len = len(text)
        text = text.reshape(batch_len, 1, self.embed_dim)

        match self.fusion_type:
            case FusionType.EARLY_WITH_IMAGE_CLS_AND_POOLING_MEAN:
                fused_embeddings = torch.cat((image, text), dim=1)
                ...
                # Since we prepend the CLS patch for each modality, the text encoding immediately follows after the last visual patch.
                text_embedding = self.to_latent(x[:, 197, :]).reshape(batch_len, 1, self.embed_dim)

                x = torch.cat((image_cls_patch_embedding, text_embedding), dim=1)
                kernel_size = x.size(1)
                x = torch.nn.functional.avg_pool2d(x, kernel_size=(kernel_size, 1)).reshape(batch_len, self.embed_dim)

                return x
            case FusionType.LATE_WITH_IMAGE_CLS_AND_POOLING_MEAN:
                ...
                # By using the image CLS patch to represent the entire image modality, we ensure equal weights for both modalities.
                x = torch.cat((image_cls_patch_embedding, text), dim=1)
...
class CentralizedModelWithAdapter(CentralizedModel):
    ...
    def __init__(self, use_pre_layer_norm, use_post_layer_norm, use_large_encoder, trainable_params_key, use_clip_encoder_for_text_embeddings, device, fusion_type, include_image_adapter, include_text_adapter, include_unified_adapter):
        super().__init__(use_pre_layer_norm, use_post_layer_norm, use_large_encoder, True, -1, trainable_params_key, use_clip_encoder_for_text_embeddings, device, fusion_type=fusion_type)
        ...
        for name, param in self.named_parameters():
            if 'classifier' in name or 'image_tokenizer' in name or 'adapter' in name:
    ...
    def forward(self, x):
        ...
        x = self.forward_with_fusion_and_adapter(text, image)
    ...
    def forward_with_fusion_and_adapter(self, text, image):
        batch_len = len(text)
        text = text.reshape(batch_len, 1, self.embed_dim)
        ...
        match self.fusion_type:
            case FusionType.EARLY_WITH_IMAGE_CLS_AND_POOLING_MEAN:
                ...
                # Since we prepend the CLS patch for each modality, the text encoding immediately follows after the last visual patch.
                text_embedding = self.to_latent(self.include_adapter_attention(x[:, 197, :], InputModality.TEXT)).reshape(batch_len, 1, self.embed_dim)
            ...
            case FusionType.LATE_WITH_IMAGE_CLS_AND_POOLING_MEAN:
                ...
                # By using the image CLS patch to represent the entire image modality, we ensure equal weights for both modalities.
                x = torch.cat((image_cls_patch_embedding, self.include_adapter_attention(text, InputModality.TEXT)), dim=1)
